...
import 'package:get/get.dart';
import '../../../presentation/view/screens/entry_point/entry_point.dart';
import '../../../presentation/view/screens/onBoarding_screen/onboarding_screen.dart';

class CustomSplashController extends GetxController {
  @override
  void onInit() {
    super.onInit();
    _navigateToNextScreen(); // Start navigation logic when the controller initializes
  }
  ...
  Future<void> _navigateToNextScreen() async {
    ...
    if (user == null) {
      ...
      // Directly go to the entry point if the user is logged in
      Get.off(() => EntryPoint(
        userEmail: user.email ?? '',
        userName: user.displayName ?? '',
        userPhotoUrl: user.photoURL ?? '',
